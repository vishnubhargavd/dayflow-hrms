...
export const WorkforceChapter: React.FC = () => {
  ...
  const [totalCount, setTotalCount] = useState<number>(148);
  const [attRate, setAttRate] = useState<string>('95.4%');
  ...
  useEffect(() => {
    ...
    const loadRealMetrics = async () => {
      try {
        const res = await fetch('http://localhost:5000/api/v1/employees?page=1&limit=10').catch(() => null);
